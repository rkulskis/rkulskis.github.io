...
It is important to note that scaling batch\_sz to match throughput is the
...
out for a new one is 2T in the best case and 4T in the worst case due
to phase-alignment; the red CHP3 is like a pipeline of rate matched
s_new->s_old->s_new. Upon swapping at CHP3 with s_new, s_old destroys
its VCPU.
